...
const AusiLkCatalogSuspenseSection = () => {
  ...
  // New state to track the currently selected category name for filtering.
  // null or empty string means "All" categories are selected.
  ...
  const allItems: ProductItem[] = (data?.items as ProductItem[]) || [];

  // Derived state for the list of categories
  const categories: Category[] = [
    {
      ...
      count:
        allItems.filter((item) => item.category === "Pharmacy Item").length ||
        0,
      color: "emerald", // green
    },
    {
      ...
      count:
        allItems.filter((item) => item.category === "Supermarket Item")
          .length || 0,
    ...
    },
    {
      ...
      count:
        allItems.filter((item) => item.category === "Electronic Item").length ||
        0,
      color: "blue", // blue
    },
    {
      ...
      count:
        allItems.filter((item) => item.category === "Gift Item").length || 0,
      color: "rose", // red/pink
    },
    {
      id: 5,
      ...
      count:
        allItems.filter((item) => item.category === "Home & Kitchen").length ||
        0,
      color: "orange", // orange
    },
    {
      ...
      count:
        allItems.filter((item) => item.category === "Beauty & Personal Care")
          .length || 0,
      color: "violet", // purple
    },
    {
      ...
      count:
        allItems.filter((item) => item.category === "Toys & Games").length || 0,
      color: "yellow", // yellow
    },
    {
      id: 8,
      ...
      count:
        allItems.filter((item) => item.category === "Sports & Outdoors")
          .length || 0,
      color: "teal", // teal
    },
  ];

  // Logic to filter products based on the selectedCategory
  const filteredProducts = allItems.filter((product) => {
    if (!selectedCategory) {
      // If no category is selected, show all items
      return true;
    }
    // Only show items that match the selected category name
  ...
  const handleAddToBucket = (product: ProductItem) => {
    addProduct({
      name: product.name,
      price: product.price,
      image: product.image,
  ...
  // Determine the heading for the Featured Products section
      ...
      <section className="bg-gradient-to-r from-amber-500 to-amber-600 dark:from-amber-600 dark:to-amber-700 text-white py-16 pt-40">
        <div className="container mx-auto px-4 text-center">
          <h1 className="text-4xl md:text-5xl font-bold mb-4">
            Home Comforts Melbourne
          </h1>
          <p className="text-xl mb-8">
            Bringing the Best of Sri Lanka and Australia to Your Door.
          </p>
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {categories.map((category) => (
              <div
                key={category.id}
                className={`category-card bg-white dark:bg-gray-700 rounded-lg shadow-md p-6 text-center cursor-pointer border hover:shadow-lg transition-all ${
                  selectedCategory === category.name
                    ? "border-amber-500 ring-2 ring-amber-500 scale-[1.02] transform" // Highlight selected category
        ...
        <div className="container mx-auto px-4">
          ...
          {filteredProducts.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {/* RENDER filteredProducts INSTEAD OF data?.items */}
